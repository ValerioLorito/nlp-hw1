...
def load_model(model, model_type):
    ...
    if device == 'cuda':
        bnb_config = BitsAndBytesConfig(
            load_in_4bit=True,
            bnb_4bit_compute_dtype=torch.float32, # Accelerate computation
        ...
        if model_type == "causal":
            model = AutoModelForCausalLM.from_pretrained(model, torch_dtype=torch.float32 if device == 'mps' else None)
        elif model_type == "seq2seq":
            model = AutoModelForSeq2SeqLM.from_pretrained(model, torch_dtype=torch.float32 if device == 'mps' else None)
        model.to(device)
    
    return model, tokenizer, device

def load_judge(model, model_type):
    tokenizer = AutoTokenizer.from_pretrained(model)
